...
public:
    ...
    Pokemon* increaseLevel() {
        if(level != maxLevel){
            ...
            auto p = this->clone(nextPokemon, level + 1);
            delete this;
            return p;
        }
    }

    /// The next function "clone" realizes a "Prototype" pattern;
    /// I create a big library (near 600 positions) of pokemons
    ...
    /// In total: it is a version of pattern "Prototype"
    /// that is parameterized by pokemonId and its level.

    Pokemon* clone(pokemonId id, int level){
        auto p = new Pokemon(*Pokedex()[id]);
        p->setLevel(level);
        return p;
    }
};
